//
// Created by milittle on 2019-03-09.
//

#ifndef ALGO_SKIPLIST_H
#define ALGO_SKIPLIST_H
...
namespace {
    ...
    private:
        ...
        node_type *head                 = nullptr;  // skiplist's head
        node_type *nil                  = nullptr;  // skiplist's nil

        static const value_type default_value; //will setting the value_type default value;
    ...
    public:
        ...
        skiplist(const skiplist& other) = delete; //拷贝构造
        ...
        skiplist& operator=(const skiplist& other) = delete; //delete赋值构造
        skiplist& operator=(skiplist&& other)      = delete; //delete移动赋值构造

    private:
        inline size_type get_random_level() const {
            return distribution(generator);  //generator range from 0 to 15
        }
    ...
    public:
        ...
        // it should be returned the node_type pointer
        ...
        std::ostream &operator<<(std::ostream & os, const skipList & list) const{
            node_type * list = head->forwards[0];
            os << "{";
